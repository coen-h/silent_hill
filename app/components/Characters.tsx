    ...
    <section id="characters" className="relative w-screen">
      ...
      <div className="max-w-[1300px] max-3xl:gap-8 max-2xl:gap-4 mx-auto grid grid-cols-3 max-xl:grid-cols-1 gap-16 px-4 py-24">
        {characters.map((character, key) => (
          <motion.div transition={{ duration: 0.8, ease: 'easeOut', delay: character.delay }} initial={{ opacity: 0, filter: "blur(8px)", y: 12 }} whileInView={{ opacity: 1, filter: "blur(0px)", y: 0 }} viewport={{ once: true, amount: 0.2}} key={key} className="h-[600px] max-xl:h-[250px] shadow-neutral-950 shadow-xl bg-black/75 relative text-center rounded-lg blur-xs overflow-hidden">
            ...
            <img className="absolute object-cover object-top inset-0 w-full h-full opacity-10 pointer-events-none" src={character.backImage} />
            ...
            <motion.img transition={{ duration: 0.8, ease: 'easeOut' }} initial={{ opacity: 0, filter: "blur(8px)"}} whileInView={{ opacity: 1, filter: "blur(0px)" }} viewport={{ once: false, amount: 0.2 }} className="absolute top-2/12 xl:left-5/10 h-[380px] -rotate-12 max-xl:opacity-15 max-xl:right-4 max-xl:pointer-events-none max-xl:hidden" src={character.weapImage}/>
          </motion.div>
        ))}
      </div>
    </section>
