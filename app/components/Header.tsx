...
export default function Header(Odata) {
  ...
  return (
    ...
    <motion.div animate={{ opacity: 1, filter: "blur(0px)" }} transition={{ duration: 0.5 }} className="flex border-x border-b border-neutral-800 bg-black/25 backdrop-blur-lg rounded-b-xl justify-between items-center top-0 z-50 py-2 px-3 max-w-[1600px] w-full opacity-0 blur-xs">
      <button className="hover:opacity-50 transition-opacity cursor-pointer" onClick={() => scrollTo('#hero')}>
        <img className="h-10" src={data.img} />
      </button>
